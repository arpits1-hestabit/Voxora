...
- Verify login and register in staging.
- Verify callback exchange and redirect.
- Verify logout clears effective session.
- Verify unauthorized API calls are rejected.
- Verify cross-user resource access fails.

## Monitoring recommendations

- Track login success and failure counts.
- Track callback and token exchange errors.
- Track unauthorized API response rates.
- Alert on spikes in auth failure events.

## Incident response basics

- Revoke compromised provider keys immediately.
- Rotate Supabase keys according to impact.
- Invalidate user sessions if required by incident scope.
- Document timeline and affected endpoints.

## Hardening roadmap

- Add auth enforcement to Deepgram temporary key route.
- Add per-user and per-IP rate limits.
- Add stricter audit logs for sensitive actions.
- Add session anomaly detection signals.

## Ownership summary

- Product auth behavior lives in app routes.
- Access isolation is guaranteed by RLS + checks.
- Session correctness depends on client type usage.
- Production safety depends on secret hygiene and monitoring.
